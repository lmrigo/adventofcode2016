var puzzleInput = `hqcfqwydw-fbqijys-whqii-huiuqhsx-660[qhiwf]
...
var day4 = function() {
  for (var i = 0; i < input.length; i++) {
    ...
    var rooms = input[i].split(/\n/)
    for (var j = 0; j < rooms.length; j++) {
      var lastDash = rooms[j].lastIndexOf('-')
      var name = rooms[j].substr(0, lastDash)
      var sectorId = rooms[j].substr(lastDash).match(/(?:-)([0-9]+)(?:\[)/)[1]
      var checksum = rooms[j].match(/(?:\[)([a-z]+)(?:\])/)[1]
      // console.log(name, sectorId, checksum)
      var letterCount = {}
      for (var k = 0; k < name.length; k++) {
        var l = name.charAt(k)
        if (l != '-') letterCount[l] !== undefined ? letterCount[l]++ : letterCount[l] = 0
      }
      var sortedLetters = Object.keys(letterCount).sort(function (a, b) {
        if (letterCount[a] > letterCount[b]) {
          return -1
        } else if (letterCount[a] < letterCount[b]) {
          return 1
        } else {
          return a.localeCompare(b)
        }
      })
      // console.log(letterCount, sortedLetters)
      if (checksum == sortedLetters.join('').substr(0,5)) {
        sumOfRealSectors += Number(sectorId)
      }
    }
      ...
      if (checksum == sortedLetters.join('').substr(0,5)) {
        ...
        for (var c = 0; c < name.length; c++) {
          ...
          if (code === 45) {
            ...
            decodedLetter = String.fromCharCode(code)
          }
          decodedName += decodedLetter
        }
        //console.log(decodedName)
        realRooms.push(decodedName + ': ' + sectorId)
        if (decodedName == 'northpole object storage') {
          northPoleSector = sectorId
        }
      }
    }
